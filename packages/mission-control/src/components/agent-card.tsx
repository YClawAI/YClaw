...
export function AgentCard({ agent, status = 'idle', currentTask, queueSize, lastOutput, sessionCount }: AgentCardProps) {
  ...
  return (
    <Link
      ...
      className={`block bg-mc-surface-hover border border-mc-border rounded p-4 border-l-2 ${borderColor} hover:border-mc-text/30 transition-colors cursor-pointer`}
    >
      ...
      <div className="flex items-center gap-3 text-[10px] text-mc-text-tertiary font-mono">
        ...
        {sessionCount != null && sessionCount > 0 && (
          <span className="text-mc-accent">{sessionCount} session{sessionCount !== 1 ? 's' : ''}</span>
        )}
